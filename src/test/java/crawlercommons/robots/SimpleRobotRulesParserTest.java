...
public class SimpleRobotRulesParserTest {
    ...
    @Test
    void testConflictingRules() throws Exception {
        BaseRobotRules rules = createRobotRules("wget", readFile("/robots/conflicting-rules.txt"));

        assertTrue(rules.isAllowed("http://domain.com/"), "conflicting-rules");
        assertFalse(rules.isAllowed("http://domain.com/foo/bar"), "conflicting-rules");

        rules = createRobotRules("testy", readFile("/robots/conflicting-rules.txt"));

        assertTrue(rules.isAllowed("http://domain.com/"), "conflicting-rules");
        assertTrue(rules.isAllowed("http://domain.com/foo/bar"), "conflicting-rules");
